...
describe('SimpleTableTest', () => {
  it('serialize', () => {
    let simpleTable = new SimpleTable('table1');
    let column1 = new SimpleColumn('table1', 'col1', new SimpleType(TypeKind.TYPE_BIGNUMERIC));
    let column2 = new SimpleColumn('table1', 'col2', new SimpleType(TypeKind.TYPE_STRING));
    simpleTable.addSimpleColumn(column1);
    simpleTable.addSimpleColumn(column2);

    let result = simpleTable.serialize();
