// Original file: src/zetasql/protos/local_service.proto

import type { EvaluateResponse as _zetasql_local_service_EvaluateResponse, EvaluateResponse__Output as _zetasql_local_service_EvaluateResponse__Output } from '../../zetasql/local_service/EvaluateResponse';

export interface EvaluateResponseBatch {
  'response'?: (_zetasql_local_service_EvaluateResponse)[];
}

export interface EvaluateResponseBatch__Output {
  'response': (_zetasql_local_service_EvaluateResponse__Output)[];
}
